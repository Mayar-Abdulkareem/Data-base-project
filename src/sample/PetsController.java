...
public class PetsController implements Initializable {
    ...
    public ObservableList<sample.petsTable> getPets() {
        ...
        try {
            ...
            String searchStr = "select Pets.PID , Pets.Category , Pets.petType,Product.P_Price , to_char(pets.BDate,'yyyy-mm-dd') , Pets.Gender from Pets , Product where Pets.PID = Product.PID ";
            ResultSet rs = statement.executeQuery(searchStr) ;
            while(rs.next()) {
                petsList.add(new petsTable(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4),rs.getString(5),rs.getString(6))) ;
            }
        }
        ...
        try {
            ...
            if(petsPriceRange!=0) {
                if( petsChoiceBoxValue.equals("All items")){
                    searchStr = "select Pets.PID , Pets.Category , Pets.petType,Product.P_Price ,to_char( Pets.BDate ,'yyyy-mm-dd'), Pets.Gender from Pets , Product where Pets.PID = Product.PID and Product.P_Price between 0 and  "+ String.valueOf(petsPriceRange);}
                else { searchStr = "select Pets.PID , Pets.Category , Pets.petType,Product.P_Price , to_char( Pets.BDate ,'yyyy-mm-dd') , Pets.Gender from Pets , Product where Pets.PID = Product.PID and Product.P_Price between 0 and  "+ String.valueOf(petsPriceRange) + " and Pets.Category = '" + petsChoiceBoxValue+"'";
                    System.out.println(petsChoiceBoxValue);
                } }

            else {
                if( petsChoiceBoxValue.equals("All items")){
                    searchStr = "select Pets.PID , Pets.Category , Pets.petType,Product.P_Price , to_char( Pets.BDate ,'yyyy-mm-dd') , Pets.Gender from Pets , Product where Pets.PID = Product.PID ";}
                else { searchStr = "select Pets.PID , Pets.Category , Pets.petType,Product.P_Price ,to_char( Pets.BDate ,'yyyy-mm-dd') , Pets.Gender from Pets , Product where Pets.PID = Product.PID  and Pets.Category = '"+petsChoiceBoxValue+"'";
                    System.out.println(petsChoiceBoxValue);
                }
    ...
    public void addToCart () {
        ...
        try {
            if(petQuantity==0){
                ...
                Connection con = u.connectOracle();
                Statement statement = con.createStatement();
                String ADD = "insert into cart_product values (Cart_product_SN.nextVal ,"+petstable.getPID()+","+LogOutController.cartID+","+petQuantity*petstable.getPrice()+","+petQuantity+")";
